#ifndef BABYBUS_CORE_INCLUDED
#define BABYBUS_CORE_INCLUDED
...
{
    float3 positionWS; // 世界空间position
    float3 positionVS; // 视口空间position
    float4 positionCS; // 齐次裁切空间position
    float4 positionNDC;// 齐次归一化设备坐标
};
...
VertexNormalInputs GetVertexNormalInputs(float3 normalOS, float4 tangentOS)
{
    ...
    /* mikkts space compliant. only normalize when extracting normal at frag.
       mikkts空间兼容。只有在提取normal at frag时才归一化。
    */
