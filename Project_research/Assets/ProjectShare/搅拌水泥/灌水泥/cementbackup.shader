...
Shader "Babybus/engineer/cementbackup"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed4 Cementcol = tex2D(_CementTex, i.uv+Timecol.r+sin(_Time.x/2));
				
				
				
				fixed4 col = lerp(Cementcol*_CementColor,Cementcol,pow(i.vcolor.x,1));
				return col;
			}
			ENDCG
		}
