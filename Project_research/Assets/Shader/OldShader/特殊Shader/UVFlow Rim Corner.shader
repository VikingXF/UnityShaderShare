...
Shader "Babybus/Unlit/UVFlow Rim Corner"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			{						
				fixed2 coord = i.uv;// +_Time.y * float2(_FlowX,_FlowY);
					
					if ((1 - coord.y) <= coord.x) {
						coord.x = coord.x;
						coord.y = 1 - (1 - coord.y) / coord.x;
					} else {
						float temp = coord.y;
						coord.y = coord.x / (1 - coord.y);
						coord.x = 1 - temp;
					}
				coord = coord + _Time.y * float2(_FlowX, _FlowY);
