...
Shader "Babybus/Water/Unilt sufacewater2"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				//采样法线贴图
				float4 normalCol = (tex2D(_NormalTex, i.uv2.zw + fixed2((_Time.x%100)*_Speed, 0)) + tex2D(_NormalTex, fixed2((_Time.x%100)*_Speed + i.uv2.w, i.uv2.z))) / 2;			
				float3 worldNormal = UnpackNormal(normalCol);
				worldNormal = lerp(half3(0, 0, 1), worldNormal, _NormalScale);
		
				// sample the texture
				float4 Maskcol = tex2D(_MaskTex, i.uv2.xy);
