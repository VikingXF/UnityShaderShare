...
namespace AlphaMask
{
	...
	public class MaskEditor : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			if (maskTarget.GetComponents<AlphaMask>().Length > 1)
			{
				GUILayout.Label("请确认添加AlphaMask脚本是否重复.");
				return;
			}
				...
				(maskTarget.GetComponent<Renderer>().sharedMaterial.mainTexture != null))
			{
                ...
                //maskTarget.maskMappingWorldAxis = (AlphaMask.MappingAxis)EditorGUILayout.EnumPopup("Mask Mapping World Axis", maskTarget.maskMappingWorldAxis);

                AlphaMask.MappingAxis maskMappingWorldAxis = (AlphaMask.MappingAxis)EditorGUILayout.EnumPopup("Mask Mapping World Axis", maskTarget.maskMappingWorldAxis);
		...
		{
            AlphaMask maskTarget = (AlphaMask)target;
			Shader maskedSpriteWorldCoordsShader = Shader.Find("Babybus/UI/Sprites Alpha Mask");
			Shader maskedUnlitWorldCoordsShader = Shader.Find("Babybus/UI/Unlit Alpha Mask");
