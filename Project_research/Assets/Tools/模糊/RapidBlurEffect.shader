...
//【2】变量声明 || Variable Declaration
		...
		struct v2f
	{
		...
		//【13】顶点着色函数 || Vertex Shader Function
	...
	{		
		...
		//计算距离中心点距离
		o.uv = v.texcoord;
		float2 dir = float2(0.5, 0.5) - o.uv;
		o.distance = saturate(pow(sqrt(dir.x * dir.x + dir.y * dir.y) + 0, 2));
		//o.distance = saturate(pow((dir.x+0.5),2));
		

		return o;
	}

	//【14】片段着色函数 || Fragment Shader Function
