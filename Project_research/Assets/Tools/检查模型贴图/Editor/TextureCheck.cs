...
public class TextureCheck  {
    ...
    static void 检查是否有真彩贴图()
    {
       ...
       // StringBuilder text = new StringBuilder();
        ...
        foreach (var guid in AssetDatabase.FindAssets("t:Texture", filePath))
        {
            ...
            {
                if (!textureImporter.DoesSourceTextureHaveAlpha() && textureImporterFormat2 != TextureImporterFormat.PVRTC_RGB4)//不带alpha通道的
                {
                    Debug.Log("IOS格式不符合:" + assetPath + ",现在格式是:" + textureImporterFormat2);
                    imageList.Add(assetPath);
                    errorList.Add("IOS格式不符合: " + ",现在格式是:" + textureImporterFormat2);
                }
                if (textureImporter.DoesSourceTextureHaveAlpha() && textureImporterFormat2 != TextureImporterFormat.PVRTC_RGBA4)//带alpha通道的
                {
                    Debug.Log("IOS格式不符合:" + assetPath + ",现在格式是:" + textureImporterFormat2);
                    imageList.Add(assetPath);
                    errorList.Add("IOS格式不符合: " + ",现在格式是:" + textureImporterFormat2);
    ...
    private static void AllFuntion(TextureImporter textureImporter)
    {
        ...
        //    Debug.Log("方法名" + nowAddButton.Name + ",属性:" + i.ParameterType);
